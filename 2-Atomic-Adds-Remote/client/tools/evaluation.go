...
func saveState(client_id string, stats Stats) error {
	...
	_, err = fmt.Fprintf(file, "REQUESTS=%d\nAVG_GET_TIME=%fms\nAVG_ADD_ATOMIC_TIME=%fms\n", stats.REQUESTS, avg_get, avg_add_atomic)
...
func IncrementAddAtomicTime(client_id string, t time.Duration, stats Stats) (int, int) {
	stats.TOTAL_ADD_ATOMIC_TIME += int(t.Nanoseconds())
	stats.REQUESTS++
	saveState(client_id, stats)
	return stats.TOTAL_ADD_ATOMIC_TIME, stats.REQUESTS
}
